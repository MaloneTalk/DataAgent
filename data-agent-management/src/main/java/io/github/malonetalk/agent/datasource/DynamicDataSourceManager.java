...
public class DynamicDataSourceManager {
    ...
    private final ConcurrentHashMap<Integer, HikariDataSource> dataSourcePool = new ConcurrentHashMap<>();

    public javax.sql.DataSource getOrCreateDataSource(Datasource datasource) {
        return dataSourcePool.computeIfAbsent(datasource.getId(), id -> createDataSource(datasource));
    }
    ...
    private HikariDataSource createDataSource(Datasource datasource) {
        DataSourceType type = DataSourceType.fromCode(datasource.getType())
                .orElseThrow(() -> new IllegalArgumentException(
                        "Unsupported datasource type: " + datasource.getType()));

        String jdbcUrl = resolveJdbcUrl(datasource, type);

        HikariConfig config = getHikariConfig(datasource, jdbcUrl, type);

        logger.info("Creating datasource pool for [{}] type={} url={}",
                datasource.getName(), type.getCode(), jdbcUrl);

        return new HikariDataSource(config);
    }

    private static HikariConfig getHikariConfig(Datasource datasource, String jdbcUrl, DataSourceType type) {
